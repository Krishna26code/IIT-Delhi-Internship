"""Amazon category loader + content embedding builder (generalizes
dataloader_beauty.py with a --category flag for Sports_and_Outdoors /
Toys_and_Games; dataloader_beauty.py is kept as-is for backward compatibility).

    meta_<Category>.json.gz -> per-item text -> sentence-t5 -> standardize ->
...
META_URLS = {
    "Beauty": "http://snap.stanford.edu/data/amazon/productGraph/categoryFiles/meta_Beauty.json.gz",
    "Sports_and_Outdoors": "http://snap.stanford.edu/data/amazon/productGraph/categoryFiles/meta_Sports_and_Outdoors.json.gz",
    "Toys_and_Games": "http://snap.stanford.edu/data/amazon/productGraph/categoryFiles/meta_Toys_and_Games.json.gz",
}


def download_meta(url: str, dest_path: Path) -> Path:
...
def load_meta(meta_path: Path) -> pd.DataFrame:
    """Parse meta_<Category>.json.gz. The lines are Python-repr-ish, so fall
    back to eval when JSON parsing fails."""
    rows: list[dict] = []
    ...
    for split in ("train.jsonl", "val.jsonl", "test.jsonl"):
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"{path} missing — run `python -m tiger.scripts.preprocess_category "
                f"--category <Category> --output-dir {sequences_dir}` first, or set "
...
def prepare(config: dict, category: str, download: bool) -> None:
    data_cfg = config["data"]

    meta_path = Path(data_cfg["meta_path"])
    if download or not meta_path.exists():
        download_meta(META_URLS[category], meta_path)

    df = load_meta(meta_path)
    print(f"[data] loaded {len(df)} {category} items")

    # Restrict to the 5-core item set the recommender uses, instead of embedding
    # the whole meta catalog. Keeps the RQ-VAE's codebook focused on items that
    # actually get SIDs and cuts encoding time substantially.
...
def main() -> None:
    ...
    ap.add_argument("--category", required=True, choices=sorted(META_URLS.keys()))
    ap.add_argument("--download", action="store_true")
    args = ap.parse_args()
    prepare(load_config(args.config), category=args.category, download=args.download)
