...
def validate(
    ...
    """Beam-decode the val set and return Recall@k / NDCG@k / invalid@k for each
    k in `ks` (defaults to the paper's (5, 10)). Pass e.g. ks=(1,5,10,15,20) with
    top_k=max(ks) to get a full K-sweep from a single beam-decode pass.
    ...
    metrics = compute_retrieval_metrics(all_pred_items, all_gold_items, ks=ks)
...
def evaluate_checkpoint(
    ...
    `ks` controls which Recall@k / NDCG@k pairs are computed; pass e.g. ks=(k,)
    together with top_k=k to get a single well-defined Recall@k / invalid-rate@k
    pair for an arbitrary k (used by tiger/scripts/plot_recall_vs_k.py)."""
    device = pick_device()
    ...
    return validate(
        ...
        beam_width=beam_width, top_k=top_k, max_batches=max_batches, ks=ks,
    )
