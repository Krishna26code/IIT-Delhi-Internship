...
from torch_geometric.data import download_google_url, extract_zip, HeteroData, InMemoryDataset
...
class AmazonReviews(InMemoryDataset, PreprocessingMixin):
    ...
    def download(self)->None:
        ...
        os.remove(path)
        folder = os.path.join(self.root, "data")
        fs.rm(self.raw_dir)
    ...
    def process(self, max_seq_len=20):
        ...
        item_data = (
            pd.DataFrame(
                [
                    meta for meta in parse(os.path.join(self.raw_dir, self.split, "meta.json.gz"))
                ]
            )
            .merge(asin2id, on="asin").sort_values(by="id").fillna({"brand": "Unknown"})
        )
