...
class PreprocessingMixin:
    @staticmethod
    def _prcess_genres(genres, one_hot=True):
        if one_hot:
            return genres
    ...
    def _generate_user_history(ratings_df,
                               ...
                               window_size: int = 200,
                               stride: int = 1,
                               train_split: float = 0.8)->torch.Tensor:
        ...
        split_grouped_by_user = PreprocessingMixin._ordered_train_test_split(grouped_by_user, "max_timestamp")
        padded_history = (
            ...
            .select(
                ...
                *(
                    ...
                    .otherwise(
                        pl.col(feat).list.slice(0, pl.col("seq_len") - 1)
                        .list.concat(
        ...
        out = {}
        out["train"] = PreprocessingMixin._df_to_tensor_dict(padded_history.filter(pl.col("is_train")), features)
        out["eval"]  = PreprocessingMixin._df_to_tensor_dict(padded_history.filter(pl.col("is_train").not_()), features)
