...
class RawMovieLens1M(MovieLens1M, PreprocessingMixin):
    ...
    def __init__(self, root, tranform=None, pre_transform=None, force_reload=False, split=None)->None:
        super(RawMovieLens1M, self).__init__(root, tranform, pre_transform, force_reload)
    ...
    def process(self, max_seq_len=None)->None:
        ...
        genres = self._prcess_genres(df["genres"].str.get_dummies("|").values, one_hot=True)
        ...
        data["user", "rating", "item"].history = self._generate_user_history(
            df, 
