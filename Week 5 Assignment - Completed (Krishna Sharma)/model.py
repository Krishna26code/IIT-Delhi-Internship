...
def _strip_dedup_col(tensor: Tensor, sem_ids_dim: int, n_layers: int)->Tensor:
...
class EncoderDecoderRetrievalModel(nn.Module):
    ...
    def __init__(self, 
        ...
        self.top_k_for_generation = top_k_for_generataion
        ...
        self.sep_token = (
            nn.Parameter(torch.randn(1, t5_d_model), requires_grad=True) if should_add_sep_token else None
        )
    ...
    def _add_repeating_offset_to_rows(self, 
                                      input_sids: Tensor,
                                      codebook_size: int,
        ...
        offsets = (
            torch.arange(self.num_hierarchies, device=input_sids.device) * codebook_size
        )
    ...
    def _inject_sep_token_between_sids(self, 
                                       id_embedding: Tensor,
                                       attention_mask: Tensor,
                                       sep_token: Tensor):
        """Inject a separator embedding between each item's token group."""
        batch_size, seq_len, emb_dim = id_embedding.size()
        item_count = seq_len // self.num_hierarchies
        reshaped_emb = id_embedding.view(batch_size, item_count, self.num_hierarchies, -1)
        reshaped_mask = attention_mask.view(batch_size, item_count, self.num_hierarchies)
        sep = sep_token.unsqueeze(0).expand(batch_size, item_count, -1).unsqueeze(-2)
        id_embedding = torch.cat([reshaped_emb, sep], dim=-2)
        attention_mask = torch.cat([reshaped_mask, reshaped_mask[:,:,[-1]]], dim=-1)
        return id_embedding.reshape(batch_size, -1, emb_dim), attention_mask.reshape(batch_size, -1)
    
    def _check_valid_prefix(self, 
                            prefix: Tensor,
                            batch_size: int = 100000)->Tensor:
        """Return a boolean mask indicating which prefixes exist in corpus codebook"""
    ...
    def encoder_forward_pass(self, attention_mask, input_ids, user_id=None):
        shifted = self._add_repeating_offset_to_rows(input_sids=input_ids,
                                                     codebook_size=self.num_embeddings_per_hierarchy,
                                                     attention_mask=attention_mask)
        input_embeds = self.item_sid_embedding_table(shifted)

        if self.sep_token is not None:
            input_embeds, attention_mask = self._inject_sep_token_between_sids(id_embedding=input_embeds,
                                                                               attention_mask=attention_mask,
                                                                               sep_token=self.sep_token)
        
        if user_id is not None and self.user_embedding is not None:
            user_embeds = self.user_embedding(torch.remainder(user_id[:,0], self.user_embedding.num_embeddings))
            input_embeds = torch.cat([user_embeds.unsqueeze(1), input_embeds], dim=1)
        ...
        encoder_output = self.encoder(inputs_embeds=input_embeds, attention_mask=attention_mask).last_hidden_state
        return encoder_output, attention_mask
    
    def decoder_forward_pass(self, attention_mask=None, future_ids=None, encoder_output=None, 
                             attention_mask_for_encoder=None, use_cache=False, past_key_values=None):
        
        if future_ids is not None:
            shifted = self._add_repeating_offset_to_rows(input_sids=future_ids, 
                                                         codebook_size=self.num_embeddings_per_hierarchy,
            ...
            if not self._is_cache_valid(past_key_values):
                ...
                input_embeds = torch.cat([bos, input_embeds], dim=1)
                if attention_mask is not None:
                    attention_mask = torch.cat(
                        [
            ...
            input_embeds = self.bos_token.unsqueeze(0).expand(encoder_output.size(0), 1, -1)
