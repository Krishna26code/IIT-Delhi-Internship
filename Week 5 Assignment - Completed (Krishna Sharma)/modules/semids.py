...
from modules.rqvae import RqVae, SeqBatch
...
def batch_to(batch, device):
    ...
    new_batch = SeqBatch(
        data["user_ids"].to(device),
        data["ids"].to(device),
        data["ids_fut"].to(device),
        data["x"].to(device),
        data["x_fut"].to(device),
        data["seq_mask"].to(device)
    )
...
class SemanticIdTokenizer(nn.Module):
    """
    Tokenizes a batch of sequences of item features into batch of sequences of semantic ids
    """
    def __init__(self,
                 input_dim: int,
                 output_dim: int,
                 ...
                 rqvae_codebook_normalize: bool = False)->None:
        super().__init__()
        self.rq_vae = RqVae(
            input_dim=input_dim,
    ...
    def _get_hits(self, query: Tensor, key: Tensor)->Tensor:
        return (rearrange(query, "b d -> b 1 d") == rearrange(key, "b d -> 1 b d")).all(axis=-1)
    ...
    def forward(self, batch: SeqBatch)->TokenizedSeqBatch:
        ...
        if self.cached_ids is None or batch.ids.max() >= self.cached_ids.shape[0]:
            ...
            sem_ids_fut = self._tokenize_seq_batch_from_cached(batch.ids_fut.unsqueeze(-1))
