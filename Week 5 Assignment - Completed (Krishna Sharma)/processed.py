...
from data.ml_1m import RawMovieLens1M
# from data.ml_32m import RawMovieLens32M
...
class ItemData(Dataset):
    def __init__(self, 
        ...
        self.item_data, self.item_text = (
            raw_data["item"]["x"][filt],
            raw_data["item"]["text"][filt]
        )
    ...
    def __getitem__(self, idx):
        item_ids = (
            torch.tensor(idx).unsqueeze(0) if not isinstance(idx, Tensor) else idx
        )
...
class SeqData(Dataset):
    def __init__(self,
        ...
        assert (not subsample) or is_train, "Can only subsample on training set"
